...
main() {
  ...
  install -d -m 0750 "${AGENTIC_ROOT}/optional/clawdbot"
  install -d -m 0750 "${AGENTIC_ROOT}/optional/clawdbot/config"
  install -d -m 0770 "${AGENTIC_ROOT}/optional/clawdbot/state"
  install -d -m 0770 "${AGENTIC_ROOT}/optional/clawdbot/logs"
  ...
  copy_if_missing "${TEMPLATE_DIR}/clawdbot.dm_allowlist.txt" "${AGENTIC_ROOT}/optional/clawdbot/config/dm_allowlist.txt" 0640
  copy_if_missing "${TEMPLATE_DIR}/mcp.tool_allowlist.txt" "${AGENTIC_ROOT}/optional/mcp/config/tool_allowlist.txt" 0640
  copy_if_missing "${TEMPLATE_DIR}/activation.request.example" "${AGENTIC_ROOT}/deployments/optional/clawdbot.request" 0640
  ...
  ensure_secret_mode "${AGENTIC_ROOT}/secrets/runtime/clawdbot.token"
  ensure_secret_mode "${AGENTIC_ROOT}/secrets/runtime/mcp.token"

  if [[ "${EUID}" -eq 0 ]]; then
    chown -R "${runtime_uid}:${runtime_gid}" \
      "${AGENTIC_ROOT}/optional/clawdbot/state" \
    ...
    if [[ -f "${AGENTIC_ROOT}/secrets/runtime/clawdbot.token" ]]; then
      chown "${runtime_uid}:${runtime_gid}" "${AGENTIC_ROOT}/secrets/runtime/clawdbot.token"
    fi
