...
async def _resolve_v1_runtime_endpoint(
    ...
    Returns:
        tuple(conversation_url, session_api_key) for V1 conversations,
        or None when the conversation is not V1.
    """
    try:
        conversation_uuid = uuid.UUID(conversation_id)
    except (ValueError, TypeError):
    ...
    return app_conversation.conversation_url.rstrip('/'), app_conversation.session_api_key


async def _forward_v1_event(
    conversation_id: str,
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f'Failed to contact V1 runtime for conversation {conversation_id}: {exc}',
        ) from exc
    ...
    if response.status_code >= 400:
        ...
        raise HTTPException(
            ...
            detail=(
                f'V1 runtime rejected event for conversation {conversation_id}: {detail}'
                if detail
                else f'V1 runtime rejected event for conversation {conversation_id}'
            ),
        )
