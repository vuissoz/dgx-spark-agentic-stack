...
class ProcessSandboxService(SandboxService):
    ...
    async def _start_agent_process(
        ...
        try:
            stdout_log_path = os.path.join(working_dir, 'agent_server.stdout.log')
            stderr_log_path = os.path.join(working_dir, 'agent_server.stderr.log')

            # Start the process
            with (
                open(stdout_log_path, 'ab') as stdout_log_file,
                open(stderr_log_path, 'ab') as stderr_log_file,
            ):
            ...
            if process.poll() is not None:
                stderr_tail = ''
                try:
                    with open(stderr_log_path, 'rb') as stderr_log_file:
                        stderr_tail = stderr_log_file.read().decode(
                            errors='replace'
                        )[-4000:]
                except Exception:
                    pass
                raise SandboxError(
                    f'Agent process failed to start. stderr tail: {stderr_tail}'
                )
    ...
    def _get_process_status(self, process_info: ProcessInfo) -> SandboxStatus:
        ...
        try:
            ...
            if process.is_running():
                ...
                # Event-loop based processes are usually reported as "sleeping".
                # Treat any live non-paused process as running and rely on the
                # HTTP /alive check in _process_to_sandbox_info for readiness.
