...
default_model_fallback="qwen3.8:27b"
...
export AGENTIC_DEFAULT_MODEL="${AGENTIC_DEFAULT_MODEL:-qwen3.8:27b}"
...
EOF
  ...
  ensure_default_export "AGENTIC_DEFAULT_MODEL" '${AGENTIC_DEFAULT_MODEL:-qwen3.8:27b}'
...
bootstrap_known_local_tools() {
  ...
  tmp_json="$(mktemp)"
  python3 - "${tool}" "${workspace}" "${state_dir}" "${agent_home}" >"${tmp_json}" <<'PY'
import json
import shutil
import sys

agent_tool = sys.argv[1]
...
commands = []
for name, purpose in [
    ("sh", "POSIX shell entrypoint used by repo-e2e tasks"),
    ("pwd", "print current working directory"),
    ("ls", "list repository files and directories"),
    ("find", "discover repository files without guessing paths"),
    ("sed", "print bounded file excerpts"),
    ("cat", "read complete small files"),
    ("git", "branch sync, status, commit, and push"),
    ("python3", "edit files or run project commands"),
    ("pytest", "run the project test suite directly when available"),
]:
...
payload = {
    "generated_by": "agent-entrypoint",
    "agent_tool": agent_tool,
    "shell": "/bin/sh",
    "workspace_root": workspace,
    "state_dir": state_dir,
    "agent_home": agent_home,
    "preferred_repo_e2e_sequence": [
        "git pull --ff-only origin <branch>",
        "pwd",
        "ls",
        "find . -maxdepth 2 -type f",
        "sed -n '1,200p' src/eight_queens.py",
        "sed -n '1,240p' tests/test_eight_queens.py",
        "python3 -m pytest -q",
        "git status",
    ],
    "commands": commands,
...
sys.stdout.write("\n")
PY
  write_if_changed "${tools_json}" "${tmp_json}"
  chmod 0600 "${tools_json}" || true

  tmp_md="$(mktemp)"
  {
    printf '# Known Local Tools\n\n'
    printf 'Generated by `agent-entrypoint` for `%s`.\n\n' "${tool}"
    printf -- '- Shell: `/bin/sh`\n'
    printf -- '- Workspace root: `%s`\n' "${workspace}"
    printf -- '- State dir: `%s`\n' "${state_dir}"
    printf -- '- JSON manifest: `%s`\n\n' "${tools_json}"
    printf 'Use only commands listed below as available. Prefer direct shell commands over invented tool schemas.\n\n'
    printf 'Recommended repo-e2e sequence:\n'
    printf '1. `git pull --ff-only origin <branch>`\n'
    printf '2. `pwd`\n'
    printf '3. `ls`\n'
    printf '4. `find . -maxdepth 2 -type f`\n'
    printf '5. `sed -n '\''1,200p'\'' src/eight_queens.py`\n'
    printf '6. `sed -n '\''1,240p'\'' tests/test_eight_queens.py`\n'
    printf '7. `python3 -m pytest -q`\n'
    printf '8. `git status`\n\n'
    printf 'Known commands:\n'
    for command_name in sh pwd ls find sed cat git python3 pytest; do
