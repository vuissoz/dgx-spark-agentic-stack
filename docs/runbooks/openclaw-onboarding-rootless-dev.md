...
```bash
./agent onboard --openclaw-init-project openclaw-default --telegram-bot-token ... --discord-bot-token ... --slack-bot-token ... --slack-app-token ...
./agent openclaw init
```
...
- `agent onboard` can pre-write `${AGENTIC_ROOT}/secrets/runtime/telegram.bot_token`, `discord.bot_token`, `slack.bot_token`, `slack.app_token`, and `slack.signing_secret`,
- `agent openclaw init` can still write `${AGENTIC_ROOT}/secrets/runtime/telegram.bot_token` directly from `--telegram-bot-token` or `--telegram-bot-token-file`,
