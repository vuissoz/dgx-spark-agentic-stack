...
- So the effective default paths become `${AGENTIC_ROOT}/agent-workspaces/<tool>/workspaces` for `claude`, `codex`, `opencode`, `vibestral`, and `hermes`.
...
- Onboarding always asks for the `git-forge` variables, because Forgejo is part of the baseline stack.

`git-forge` variables:

| Variable | Allowed values | Default | Stored in |
|---|---|---|---|
| `GIT_FORGE_HOST_PORT` | free TCP port on the host | `13010` | shell |
| `GIT_FORGE_ADMIN_USER` | non-empty login | `system-manager` | shell |
| `GIT_FORGE_SHARED_NAMESPACE` | organization/group slug | `agentic` | shell |
| `GIT_FORGE_ENABLE_PUSH_CREATE` | `0` or `1` | `0` | shell |

`git-forge` runtime secrets:
...
| `AGENTIC_CONTEXT_COMPACTION_DANGER_PERCENT` | integer `1..99` (`> soft`) | `90` | shell, `runtime.env` |
...
| `AGENTIC_KILOCODE_CLI_NPM_SPEC` | npm package spec | `@kilocode/cli@latest` | shell, `runtime.env` |
...
| `AGENTIC_HERMES_AGENT_GIT_URL` | Git URL | `https://github.com/NousResearch/hermes-agent.git` | shell, `runtime.env` |
| `AGENTIC_HERMES_AGENT_GIT_REF` | Git ref | `v2026.4.3` | shell, `runtime.env` |
| `AGENTIC_HERMES_AGENT_GIT_SHA` | Git commit SHA | `abf1e98f6253f6984479fe03d1098173a9b065a7` | shell, `runtime.env` |
| `AGENTIC_HERMES_PIP_EXTRAS` | comma list of pip extras | `pty,cli` | shell, `runtime.env` |
...
- `AGENTIC_AGENT_NO_NEW_PRIVILEGES=false` enables in-container `sudo` mode for `agentic-{claude,codex,opencode,vibestral,hermes}` (`./agent sudo-mode on`) with an explicit hardening tradeoff.
...
| `RAG_LEXICAL_BACKEND` | `disabled` or `opensearch` | `disabled` (`opensearch` with `rag-lexical`) | shell, `runtime.env` |
...
Notes:
- `COMPOSE_PROFILES=rag-lexical` starts OpenSearch and selects `RAG_LEXICAL_BACKEND=opensearch` by default.
