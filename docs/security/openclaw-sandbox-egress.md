...
- The `openclaw` + `openclaw-sandbox` services shipped by this repository implement the core OpenClaw API/sandbox pair.
- This repository also ships `openclaw-gateway` for upstream OpenClaw Web UI/WS access on loopback host bind (`127.0.0.1:${OPENCLAW_GATEWAY_HOST_PORT:-18789}`).
