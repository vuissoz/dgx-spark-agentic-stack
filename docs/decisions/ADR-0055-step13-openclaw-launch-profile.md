...
This ADR still documents the launch-inspired profile contract, but the runtime path now lives under `${AGENTIC_ROOT}/openclaw/config/...`.
...
- Extend OpenClaw runtime service to:
...
- Extend validations:
  - `agent up core` validates OpenClaw profile presence/schema before deploy,
...
## Consequences
- OpenClaw now has an explicit versioned contract artifact that operators can audit and diff.
