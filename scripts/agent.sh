  ...
  case "$1" in
    ...
    optional-pi-mono)
      printf '%s\n' \
        ...
        "${AGENTIC_REPO_ROOT}/deployments/images/agent-cli-base/whisper-en.sh" \
...
agent_base_build_fingerprint() {
...

  {
    ...
    if [[ "${dockerfile_real}" == "${default_dockerfile_real}" ]]; then
      ...
      [[ -f "${AGENTIC_REPO_ROOT}/deployments/images/agent-cli-base/whisper-en.sh" ]] || die "default whisper-en wrapper missing"
      ...
      sha256sum "${AGENTIC_REPO_ROOT}/deployments/images/agent-cli-base/whisper-en.sh"
...
assert_agent_base_image_contract() {
  ...
  timeout 30 docker run --rm --entrypoint sh "${image_ref}" -lc 'command -v bash tmux git curl ffmpeg vlc whisper whisper-en whisper-fr >/dev/null' \
    || die "agent base image must include bash/tmux/git/curl/ffmpeg/vlc/whisper/whisper-en/whisper-fr: ${image_ref}"
  ...
  while IFS= read -r line || [[ -n "${line}" ]]; do
    ...
    case "${key}" in
      ...
      COMPOSE_PROFILES)
        ...
        ;;
      *)
        if agentic_runtime_schema_has_role "${key}" load; then
          export "${key}=${value}"
        fi
        ;;
    esac
...
ensure_runtime_env() {
  ...
  while IFS='|' read -r key label; do
    if [[ "${key}" == "AGENTIC_OLLAMA_MODELS_TARGET_DIR" ]] && [[ -z "${AGENTIC_OLLAMA_MODELS_TARGET_DIR:-}" ]]; then
      value="${OLLAMA_MODELS_DIR}"
    elif [[ "${key}" == "AGENTIC_GPU_CLOCK_LOCK" ]] && [[ -z "${AGENTIC_GPU_CLOCK_LOCK:-}" ]]; then
      value="default"
    else
      value="${!key-}"
    fi
    set_runtime_env_value "${key}" "${value}"
  done < <(agentic_runtime_schema_iter_role persist)
}
