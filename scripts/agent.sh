...
Usage:
  ...
  agent prereqs
  agent onboard [runtime flags...] [--openwebui-admin-email ... --openwebui-admin-password ... --default-model ... --grafana-admin-user ... --grafana-admin-password ... --openhands-llm-model ... --allowlist-domains ... --optional-modules ... --output ... --non-interactive --require-complete]
...
agent_workspace_dir() {
  local tool="$1"
  printf '%s\n' "${AGENTIC_AGENT_WORKSPACES_ROOT}/${tool}/workspaces"
}
...
ensure_runtime_env() {
  ...
  local -a keys=(
    ...
    "AGENT_DEFAULT_PROJECT=${AGENT_DEFAULT_PROJECT}"
...
cmd_profile() {
  ...
  printf 'agent_default_project=%s\n' "${AGENT_DEFAULT_PROJECT}"
...
detect_project_name() {
  ...
  elif [[ -n "${AGENT_DEFAULT_PROJECT:-}" ]]; then
    project="${AGENT_DEFAULT_PROJECT}"
...
purge_directory_contents() {
  local path="$1"
  [[ -n "${path}" ]] || return 0
  [[ "${path}" == "${AGENTIC_ROOT}"* || "${path}" == "${AGENTIC_AGENT_WORKSPACES_ROOT}"* ]] \
    || die "refusing to purge path outside AGENTIC_ROOT/AGENTIC_AGENT_WORKSPACES_ROOT: ${path}"
