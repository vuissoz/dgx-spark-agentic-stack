...
cmd_gpu_clock() {
  ...
  local actor="${SUDO_USER:-${USER:-unknown}}"
  local configured_lock="${AGENTIC_GPU_CLOCK_LOCK:-default}"
  local query_output=""
  ...
  case "${action}" in
    status)
      printf 'gpu_clock_configured=%s\n' "${configured_lock}"
      if query_output="$(nvidia-smi --query-gpu=clocks.gr,clocks.max.gr,pstate --format=csv,noheader 2>/dev/null)"; then
        printf '%s\n' "${query_output}"
      else
    ...
    low)
      ensure_runtime_env
      set_runtime_env_value "AGENTIC_GPU_CLOCK_LOCK" "${AGENT_GPU_CLOCK_LOW_PRESET}"
      AGENTIC_GPU_CLOCK_LOCK="${AGENT_GPU_CLOCK_LOW_PRESET}"
      export AGENTIC_GPU_CLOCK_LOCK
      run_host_root_command nvidia-smi -lgc "${AGENT_GPU_CLOCK_LOW_PRESET}"
      append_changes_log "gpu clock actor=${actor} action=lock value=${AGENT_GPU_CLOCK_LOW_PRESET}"
      printf 'gpu-clock locked=%s\n' "${AGENT_GPU_CLOCK_LOW_PRESET}"
      ;;
    reset)
      ensure_runtime_env
      set_runtime_env_value "AGENTIC_GPU_CLOCK_LOCK" "default"
      AGENTIC_GPU_CLOCK_LOCK="default"
      export AGENTIC_GPU_CLOCK_LOCK
      run_host_root_command nvidia-smi -rgc
      append_changes_log "gpu clock actor=${actor} action=reset"
      printf 'gpu-clock reset=default\n'
      ;;
    *)
      die "Usage: agent gpu-clock [status|low|reset]"
      ;;
  esac
}
...
ensure_runtime_env() {
  ...
  local -a keys=(
    ...
    "OPENWEBUI_HOST_PORT=${OPENWEBUI_HOST_PORT}"
    "OPENHANDS_HOST_PORT=${OPENHANDS_HOST_PORT}"
    "COMFYUI_HOST_PORT=${COMFYUI_HOST_PORT}"
    "PROMETHEUS_HOST_PORT=${PROMETHEUS_HOST_PORT}"
    "GRAFANA_HOST_PORT=${GRAFANA_HOST_PORT}"
    "LOKI_HOST_PORT=${LOKI_HOST_PORT}"
    "GIT_FORGE_HOST_PORT=${GIT_FORGE_HOST_PORT}"
    "GIT_FORGE_SSH_HOST_PORT=${GIT_FORGE_SSH_HOST_PORT}"
    "PORTAINER_HOST_PORT=${PORTAINER_HOST_PORT}"
    "OPENCLAW_WEBHOOK_HOST_PORT=${OPENCLAW_WEBHOOK_HOST_PORT}"
    "OPENCLAW_GATEWAY_HOST_PORT=${OPENCLAW_GATEWAY_HOST_PORT}"
    "OPENCLAW_RELAY_HOST_PORT=${OPENCLAW_RELAY_HOST_PORT}"
    ...
    "AGENTIC_GPU_CLOCK_LOCK=${AGENTIC_GPU_CLOCK_LOCK:-default}"
    ...
    "AGENTIC_CONTEXT_COMPACTION_SOFT_PERCENT=${AGENTIC_CONTEXT_COMPACTION_SOFT_PERCENT}"
    "AGENTIC_CONTEXT_COMPACTION_DANGER_PERCENT=${AGENTIC_CONTEXT_COMPACTION_DANGER_PERCENT}"
    "AGENTIC_CONTEXT_BUDGET_TOKENS=${AGENTIC_CONTEXT_BUDGET_TOKENS}"
    "AGENTIC_CONTEXT_COMPACTION_SOFT_TOKENS=${AGENTIC_CONTEXT_COMPACTION_SOFT_TOKENS}"
    "AGENTIC_CONTEXT_COMPACTION_DANGER_TOKENS=${AGENTIC_CONTEXT_COMPACTION_DANGER_TOKENS}"
...
cmd_profile() {
  ...
  printf 'context_compaction_soft_percent=%s\n' "${AGENTIC_CONTEXT_COMPACTION_SOFT_PERCENT}"
  printf 'context_compaction_danger_percent=%s\n' "${AGENTIC_CONTEXT_COMPACTION_DANGER_PERCENT}"
  ...
  printf 'trtllm_nvfp4_hf_repo=%s\n' "${TRTLLM_NVFP4_HF_REPO}"
  printf 'trtllm_nvfp4_hf_revision=%s\n' "${TRTLLM_NVFP4_HF_REVISION}"
  printf 'trtllm_nvfp4_prepare_enabled=%s\n' "${TRTLLM_NVFP4_PREPARE_ENABLED}"
...
case "$cmd" in
  ...
  tunnel)
    shift
    [[ -x "${AGENT_TUNNEL_SCRIPT}" ]] || die "tunnel script missing or not executable: ${AGENT_TUNNEL_SCRIPT}"
