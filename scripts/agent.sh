  ...
  while IFS= read -r line || [[ -n "${line}" ]]; do
    ...
    case "${key}" in
      OLLAMA_CONTAINER_MODELS_PATH)
        ...
        ;;
      AGENTIC_OLLAMA_MODELS_LINK|AGENTIC_OLLAMA_MODELS_TARGET_DIR|OLLAMA_MODELS_DIR|OLLAMA_CONTAINER_USER|QDRANT_CONTAINER_USER|OLLAMA_MODELS_MOUNT_MODE|OLLAMA_PRELOAD_GENERATE_MODEL|OLLAMA_PRELOAD_EMBED_MODEL|OLLAMA_MODEL_STORE_BUDGET_GB|RAG_EMBED_MODEL)
        export "${key}=${value}"
        ;;
      *)
        ;;
