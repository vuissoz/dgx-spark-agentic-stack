...
profile_output="$(
  ...
  TRTLLM_NVFP4_HF_REPO='nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-NVFP4' \
...
printf '%s\n' "${profile_output}" | grep -q '^trtllm_models=https://huggingface.co/nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-NVFP4$' \
...
printf '%s\n' "${profile_output}" | grep -q '^trtllm_nvfp4_hf_repo=nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-NVFP4$' \
