...
if [[ "${EUID}" -eq 0 ]]; then
  chown "${AGENT_RUNTIME_UID:-1000}:${AGENT_RUNTIME_GID:-1000}" "${agentic_root}/secrets/runtime/openclaw.token"
fi
...
assert_no_public_bind 8111 || fail "openclaw service must not expose host port 8111"
